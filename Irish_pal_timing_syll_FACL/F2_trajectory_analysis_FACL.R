...
################
# Correlate F2 at VC transition with COG of stop release burst.

# Read in data
load("cog_measures.Rdata")
# setwd("")
# cog.measures <- read.csv("release_log.txt",sep="\t")
# cog.measures <- cog.measures %>% rename("filename" = "Filename")

# Merge COG measures with time-normalized F2 data

# Get F2 at endpoint, here defined as the mean F2 over the final 10% of the vowel.
# f2.at.end <- formants.trimmed %>% filter(formant == "F2") %>% group_by(token.code) %>% filter(step == 100)
f2.at.end <- formants.trimmed %>% filter(formant == "F2") %>% group_by(token.code) %>%
             filter(step >= 90) %>% mutate(log.Fx.norm = mean(log.Fx.norm,na.rm=T)) %>% 
             select(-c(time,step,freq,log.Fx)) %>% distinct() # Get rid of individual steps now that you've averaged.
f2.at.end

f2.cog <- left_join(f2.at.end,
                    cog.measures,
                    by="filename")
# f2.cog


# Get by-condition correlations between F2, COG,
# and test for significance.
dur.cors <- f2.cog %>% group_by(v,c.place,sec.art) %>%
                       summarize(DurCor = round(cor.test(log.Fx.norm,cog)$estimate,digits=3),
                                 DurSig = cor.test(log.Fx.norm,cog)$p.value)

dur.cors <- dur.cors %>% mutate(printCor = case_when(abs(DurSig) <= 0.05 ~ paste0(DurCor,"*"),
                                                     .default = as.character(DurCor)
                                                     )
                                )

dur.cors.sig <- dur.cors %>% filter(abs(DurSig) <= 0.05)
dur.cors.sig

dur.cors.sig <- dur.cors.sig %>% mutate(pl.sec = paste0(c.place,
                                                        str_replace(sec.art,"C","")
                                                        )
                                        )
dur.cors.sig

# Plot
cor.f2.cog.plt <- ggplot(data=f2.cog,
                         aes(x=log.Fx.norm,
                             y=cog,
                         color=sec.art,lty=sec.art)
                         )+
  scale_color_manual(values=colorSet)+
  geom_point(size=0.75,alpha=0.25)+
  geom_smooth(method="lm",lwd=1.25)+
  geom_label(inherit.aes=F,
             data=dur.cors.sig %>% filter(sec.art == "Cˠ"),
             aes(x=-0.3,y=1000,
                 label = paste0("/",pl.sec,"/: ",DurCor)),
             # Using printCor gets you significance, but for some plots that seems misleading,
             # because r^2 is so low.
             parse = F,
             size=3
             )+
  geom_label(inherit.aes=F,
             data=dur.cors.sig %>% filter(sec.art == "Cʲ"),
             aes(x=-0.3,y=8500,
                 label = paste0("/",pl.sec,"/: ",DurCor)),
             # Using printCor gets you significance, but for some plots that seems misleading,
             # because r^2 is so low.
  ...
  xlab("Normalized F2 at V end in VC\n(final 10% of V)")+
  ylab(paste0("Center of gravity (Hz) at\nC release in VC (",unique(f2.cog$reldur),"ms window)"))+
  facet_grid(v~c.place)
...
# Save w/ cairo
setwd("")
cairo_pdf(file="lognorm_F2_COG_COR.pdf",
          width=10,height=6)
  cor.f2.cog.plt
dev.off()
...
##########
# Same plot, but don't facet by vowel.

# Get by-condition correlations between F2, COG,
# and test for significance.
dur.cors <- f2.cog %>% group_by(c.place,sec.art) %>%
  summarize(DurCor = round(cor.test(log.Fx.norm,cog)$estimate,digits=3),
            DurSig = cor.test(log.Fx.norm,cog)$p.value)
...
# Plot
cor.f2.cog.plt.nov <- ggplot(data=f2.cog,
                         aes(x=log.Fx.norm,
                             y=cog,
                             color=sec.art,lty=sec.art)
                         )+
...
# Save w/ cairo
setwd("")
cairo_pdf(file="lognorm_F2_COG_COR_nov.pdf",
          width=10,height=4)
  cor.f2.cog.plt.nov
dev.off()
